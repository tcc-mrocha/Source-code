...
node_nome = "node-sala" -- nomde do dispositivo
host = "192.168.0.50" -- endereco do servidor mqtt broker (mosquitto) rodando no raspberry pi
porta_mqtt = 1883 -- porta onde roda o mosquitto
top_base = "tccmrocha/sala" -- topico base do dispositivo
...
pino_DHT22 = 4 -- seta o pino 4 para o sensor de temperatura/umidade (DHT22)
...
function leDHT22()
    ...
    table.insert(topico, top_base .. "/temperatura")
    table.insert(valor,  temperatura)
    table.insert(topico, top_base .. "/umidade")
    table.insert(valor,  umidade)   
    print("\nLendo DHT22..." .. "\n  Temperatura = " .. temperatura .. "\n  Umidade = " .. umidade .. "\n")
end

-- cria o cliente mqtt com as opcoes especificadas
mqtt_cliente = mqtt.Client(nodeNome, 120, "marcelo", "7036c004f")
...
mqtt_cliente:on(
    ...
    function(client, topic, data)
        print (topic .. " = " .. data)
        if topic == top_base .. "/umidificador" and data == "liga" then
            gpio.write(pino_umidade, gpio.HIGH) -- liga o umidificador
        end
        if topic == top_base .. "/umidificador" and data == "desliga" then
            gpio.write(pino_umidade, gpio.LOW) -- desliga o umidificador
        end
        if topic == top_base .. "/arcondicionado" and data == "liga" then
            gpio.write(pino_arcon, gpio.HIGH) -- liga o ar condicionado
        end
        if topic == top_base .. "/arcondicionado" and data == "desliga" then
        ...
        if topic == "tccmrocha/all/pergunta" and data == "ping" then
            ...
            mqtt_cliente:publish("tccmrocha/all/resposta/sala", wifi.sta.getip().." - Online", 0, 0, function() 
	    ...
	    function(client) -- se a conexao foi um sucesso entao assina os topicos
	        ...
	        client:subscribe(top_base .. "/luz", 0, function(client) 
	            ...
	            client:subscribe(top_base .. "/luz/intensidade", 0, function(client) 
	                ...
	                client:subscribe(top_base .. "/arcondicionado", 0, function(client) 
	                    print("Topico: " .. top_base .. "/arcondicionado OK")
	                    client:subscribe(top_base .. "/umidificador", 0, function(client) 
	                        print("Topico: " .. top_base .. "/umidificador OK")
